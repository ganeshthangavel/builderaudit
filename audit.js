...
/* Shown above the progress card. Lets the user enter their email so we can
   mail them the report when the audit finishes (5-10 min for hostile sites).
   They can close the tab and come back, or skip and wait. */
.sc-notify {
  background: #fff;
  border: 3px solid var(--line);
  border-radius: 14px;
...
.sc-notify-sub {
  ...
  color: var(--ink); opacity: 0.78; line-height: 1.5;
  margin: 0 0 16px;
}
.sc-notify-row {
...
.sc-notify-input:focus { border-color: var(--accent); }
.sc-notify-btn {
  padding: 12px 22px;
  background: var(--accent); color: #fff;
  border: 2px solid var(--line); border-radius: 6px;
...
.sc-notify-btn:hover { transform: translate(-1px,-1px); box-shadow: 5px 5px 0 var(--line); }
.sc-notify-btn:active { transform: translate(2px,2px); box-shadow: 1px 1px 0 var(--line); }
.sc-notify-btn[disabled] { opacity: 0.7; cursor: default; transform: none; box-shadow: 4px 4px 0 var(--line); }
.sc-notify-skip {
...
.sc-notify-error {
  margin-top: 10px;
  font-family: var(--body); font-size: 13px; color: var(--bad);
}
@media (max-width: 700px) {
  .sc-notify { padding: 18px 18px; box-shadow: 4px 4px 0 var(--line); }
  .sc-notify-title { font-size: 18px; }
  .sc-notify-sub { font-size: 13.5px; }
  .sc-notify-btn { width: 100%; }
}
</style>
</head>
<body>
...
<main class="sc-main">
  <div class="sc-wrap">
    ...
    <!-- Email capture banner: lets the user opt-in to "email me when ready"
         instead of waiting on the page. Hidden once they submit OR audit completes. -->
    <div class="sc-notify" id="notifyCard">
      <h2 class="sc-notify-title">This usually takes 5–10 minutes</h2>
      <p class="sc-notify-sub">Don't fancy staring at a progress bar? Drop your email and we'll send the report straight to you when it's done. You can close this tab safely.</p>
      <div class="sc-notify-row">
        <input type="email" class="sc-notify-input" id="notifyEmail" placeholder="your@email.com" autocomplete="email">
        <button class="sc-notify-btn" id="notifyBtn">Email me when ready</button>
      </div>
      <div class="sc-notify-error" id="notifyError" style="display:none"></div>
      <button class="sc-notify-skip" id="notifySkip">No thanks, I'll wait here</button>
    </div>

    <!-- Success state for the email capture — replaces the form after submit -->
    <div class="sc-notify" id="notifySent" style="display:none">
      <div class="sc-notify-success">
        <div class="sc-notify-success-icon">✓</div>
        <div>We'll email <strong id="notifySentEmail"></strong> when your report is ready. Feel free to close this tab.</div>
      </div>
    </div>
...
var currentAuditId = null;  /* Set when server sends 'audit_id' SSE event */
...
function handle(name, data) {
  if (name === 'audit_id') {
    /* Server sends this first so we know the ID for the email-notify POST */
    ...
    /* Hide notify banner — they're about to be redirected to the report anyway */
    var nc = document.getElementById('notifyCard'); if (nc) nc.style.display = 'none';
    var ns = document.getElementById('notifySent'); if (ns) ns.style.display = 'none';
...
(function setupNotify(){
  ...
  var errBox = document.getElementById('notifyError');
  var skipBtn = document.getElementById('notifySkip');
  var card = document.getElementById('notifyCard');
  var sent = document.getElementById('notifySent');
  var sentEmail = document.getElementById('notifySentEmail');
  ...
  async function submitNotify() {
    ...
    /* Basic format check — match the server's regex */
    if (!/^\S+@\S+\.\S+$/.test(email)) {
      showError('Please enter a valid email address.');
      input.focus();
      return;
    }
    /* If we don't have the audit ID yet, the server hasn't sent it. Wait a beat and retry. */
    if (!currentAuditId) {
      /* Poll up to 5 seconds for the ID, in case SSE hasn't delivered it yet */
      var waited = 0;
      while (!currentAuditId && waited < 5000) {
        await new Promise(function(r){ setTimeout(r, 200); });
        waited += 200;
      }
      if (!currentAuditId) {
        showError('Still starting up — try again in a couple of seconds.');
        return;
      }
    }
    btn.disabled = true;
    ...
    try {
      var res = await fetch('/api/audit/notify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ auditId: currentAuditId, email: email }),
      });
      if (!res.ok) {
      ...
      showError(err.message || 'Could not save your email.');
      btn.disabled = false;
      btn.textContent = 'Email me when ready';
    }
  }

  btn.addEventListener('click', submitNotify);
