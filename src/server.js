...
app.post('/api/audit', async (req, res) => {
  ...
  /* Grant whoever runs this audit the right to view the report they just
   * generated — even if they're not logged in. The report-data endpoint checks
   * for this cookie (audit_unlock_<id>); without it, a logged-out runner gets a
   * locked teaser and the dashboard falls back to its empty state. Set BEFORE
   * the SSE headers/stream start, since cookies must go out with the headers. */
  res.cookie('audit_unlock_' + auditId, '1', {
    httpOnly: true,
    sameSite: 'lax',
...
app.listen(PORT, '0.0.0.0', () => console.log('Server running on port ' + PORT));const express = require('express');
const cors = require('cors');
const path = require('path');
const cookieParser = require('cookie-parser');
const https = require('https');
